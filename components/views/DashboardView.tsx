...
export default function DashboardView({ onNavigate }: { onNavigate: (t: TabType, priority?: PriorityFilterType, mTab?: MaintenanceMacroTab) => void }) {
  const { sectors, sectorItemsMap, shoppingStats, maintenanceStats, priorityItems } = useApp();
        ...
        >
          ...
          <div className="flex items-center gap-4 mt-auto">
            <div className="w-16 h-16 rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800 shrink-0 border border-slate-200 dark:border-slate-700">
              <Image 
                src={nextBigBuy?.img || VACUUM_IMG} 
                alt={nextBigBuy?.name || "Produto Prioritário"} 
                width={64} 
                height={64} 
            ...
            <div className="flex-1 min-w-0">
              <h4 className="text-base font-semibold text-slate-900 dark:text-slate-100 truncate">
                {nextBigBuy?.name || 'Aspirador Robô Pro'}
              </h4>
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5 truncate">
                {nextBigBuy?.category ? `Categoria ${nextBigBuy.category}` : 'Planejado para este ciclo'} • {nextBigBuy?.qty || 1} unidade(s)
              </p>
            </div>
            <div className="text-right shrink-0">
              <div className="text-lg font-bold text-slate-900 dark:text-slate-100">
                {nextBigBuy ? `R$ ${(nextBigBuy.numPrice * nextBigBuy.qty).toFixed(2).replace('.', ',')}` : 'R$ 299,00'}
              </div>
